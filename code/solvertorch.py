...
def rigidTransCustom(source, target):
    """
    Registers two point clouds by rigid transformation using Singular Value Decomposition (SVD).
    target_x = target_T_source * source_x
    """
    assert len(source) == len(target), "Source and target must have the same number of points."
    
    # Compute centroids of the source and target point clouds
    source_centroid = np.mean(source, axis=0)
    target_centroid = np.mean(target, axis=0)
    
    # Center the point clouds around their centroids
    ...
    # Handle the case where the determinant of R is -1 (reflection)
...
# Reverse rigidTrans transformation
...
def solveminimize(target_keypoints, image, init_pose=None, init_shape=None,IsLeft=True):
    ...
    mTc = rigidTransCustom(palm(target_keypoints_scaled),palm(mano_keypoints))
    print(mTc)
    ...
    rr.log("real_SCALED", rr.Points3D(target_keypoints_scaled, colors=(0, 0, 255),labels=[str(i) for i in range(21)], radii=0.001))#show MP real keypoints
    ...
    anatomy_loss.setup()
    axis_layer = AxisLayerFK(mano_assets_root="../")
    ...
    def objective_functionMSEAnatomy(params, mano_layer, target_keypoints, progress=True):
        ...
        if progress:
            ...
            target_fingertips = target_keypoints[fingertip_indices, :]
            mano_fingertips = J[fingertip_indices, :]
            ...
            # GMLoss = GMLossCalc(J-torch.tensor(target_keypoints, dtype=torch.float32),rho = 1)
            # HuberLoss = huber_loss(J,torch.tensor(target_keypoints, dtype=torch.float32),delta=1.0)

            mse_loss_fingertips = torch.nn.functional.mse_loss(mano_fingertips, torch.tensor(target_fingertips, dtype=torch.float32))

            # Weighted combination of losses
            total_loss = 1 * mse_loss + 4* mse_loss_fingertips 
    ...
    return mano_layer(correct_root_optimized_hand_pose, optimized_betas), correct_root_optimized_hand_pose, optimized_betas, reversed_mano_keypoints
