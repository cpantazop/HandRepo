...
def residuals_function(params, mano_layer, target_keypoints,progress=True):
    hand_pose_np = params[:48].reshape((1, -1))
    betas_np = params[48:].reshape((1, -1))

    # Convert to tensors
    hand_pose_tensor = torch.tensor(hand_pose_np, dtype=torch.float32, requires_grad=False)
    betas_tensor = torch.tensor(betas_np, dtype=torch.float32, requires_grad=False)
    ...
    # Compute residuals
    residuals = J - torch.tensor(target_keypoints, dtype=torch.float32)
    return residuals.detach().numpy().flatten()
...
def solvedogleg(target_keypoints,image,init_pose=None,init_shape=None):
    ...
    mTc = rigitTrans(palm(target_keypoints),palm(mano_keypoints))
    print(mTc)

    target_keypoints_trans = np.dot(
        mTc, np.vstack((target_keypoints.T, np.ones(len(target_keypoints))))
    )
    ...
    target_palm_size = np.linalg.norm(target_keypoints_trans[0] - target_keypoints_trans[5])
    scaling_factor = mano_palm_size / target_palm_size

    # Scale target keypoints
    target_keypoints_scaled = target_keypoints_trans * scaling_factor

    mTc0 = rigitTrans(findzero(mano_keypoints),findzero(target_keypoints_scaled))
    translation_vector0 = mTc0[:3, 3]

    target_keypoints_scaled = target_keypoints_scaled - translation_vector0
    ...
    rr.log("real_translatedSCALED", rr.Points3D(target_keypoints_scaled, colors=(255, 255, 255),labels=[str(i) for i in range(21)], radii=0.001))#show MP real keypoints
    ...
    # Optimize with Dogleg
    result = least_squares(
        fun=residuals_function,
        x0=initial_guess,
        args=(mano_layer, target_keypoints_trans,True),  # Extra arguments for residuals_function
        method="dogbox",  # Dogleg optimization method
        # method="trf",  # trf optimization method
    )
    ...
    reversed_mano_keypoints = (1 / scaling_factor)*(Jnum  + translation_vector0)
    reversed_mano_keypoints = reverse_rigid_transformation(reversed_mano_keypoints, mTc)
    ...
    reversed_vertices = (1 / scaling_factor) * (V + translation_vector0)

    # Reverse the rigid transformation
    reversed_vertices = reverse_rigid_transformation(reversed_vertices, mTc)

    rr.log("estimatedfinalMeshBIG",rr.Mesh3D(vertex_positions=reversed_vertices,triangle_indices=F,vertex_colors=(255, 0, 0,5)))
    ...
    rr.log(
        ...
        rr.LineStrips2D(
            ...
            radii=[0.2 for _ in all_edges],  # Thickness for each line
            # labels=[f"Edge {i}" for i in range(len(all_edges))],  # Optional: label each edge
        )
    )

    return mano_layer(optimized_hand_pose, optimized_betas),optimized_hand_pose, optimized_betas


def solveminimize1(target_keypoints, image, init_pose=None, init_shape=None,IsLeft=True):
    mano_layer = ManoLayer(
        rot_mode="axisang",
        use_pca=False,
    ...
    def objective_functionMSEAnatomy(params, mano_layer, target_keypoints, progress=True):
        ...
        # Calculate losses
            ...
            total_loss = 1 * mse_loss #+ 1* anatomy_constraint
    ...
    result = minimize(
        fun = lambda params: objective_functionMSEAnatomy(params, mano_layer, target_keypoints_scaled),
        x0=initial_guess,
...
def solveminimize(target_keypoints, image, init_pose=None, init_shape=None,IsLeft=True):
    ...
    # # Compact representation: axis * angle
    # axis_angle = (axis * angle).tolist() # This is your 3-value Axis-Angle representation
    # print("Axis-Angle :", axis_angle) 

    # root_pose = torch.tensor([axis_angle]).repeat(BS, 1)
    # finger_pose = torch.zeros(BS, 45)
    # hand_pose = torch.cat([root_pose, finger_pose], dim=1)
    # hand_pose.requires_grad_(True)

    # mano_output: MANOOutput = mano_layer(hand_pose, betas_shape)
    # mano_keypoints = mano_output.joints[0]
    # mano_keypoints = mano_keypoints.detach().numpy()