...
def process_and_evaluate_images_from_dexter_object():
    ...
    do = DexterObjectDataset(
        data_root="C:/Users/chris/Documents/GitHub/HandRepo/data/dexter+object",
        data_split='test',
        hand_side='left',
        njoints=21,
        use_cache=False,
    ...
    # for id in tqdm(range(10)):
        ...
        gt_landmarks = np.array(data['tip'])  # 3D ground truth coordinates (fingertips)
        intrinsics = do.color_intrisics  # Intrinsics matrix from EgoDexter
        ...
        print(f"Image {id}: gt_landmarks_valid  = {gt_landmarks_valid}, result_fingertips_cam = {result_fingertips_cam}")
    ...
    output_file = os.path.join(output_dir, "metrics_DO_H.csv")
...
process_and_evaluate_images_from_dexter_object()
