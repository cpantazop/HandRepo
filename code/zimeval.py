...
class EvalUtil:
    ...
    def feed(self, keypoint_gt, keypoint_pred, keypoint_vis=None):
        ...
        keypoint_gt = np.atleast_2d(np.squeeze(keypoint_gt))
        keypoint_pred = np.atleast_2d(np.squeeze(keypoint_pred))

        if keypoint_vis is None:
            keypoint_vis = np.ones(keypoint_gt.shape[0], dtype=bool)  # Always 1D
        else:
            # If keypoint_vis is provided, reshape it to 1D if needed
            keypoint_vis = np.array(keypoint_vis, dtype=bool).reshape(-1)  # Force 1D
        # keypoint_vis = np.squeeze(keypoint_vis).astype("bool")

        print(f"keypoint_gt.shape = {keypoint_gt.shape}")
        print(f"keypoint_pred.shape = {keypoint_pred.shape}")
        print(f"keypoint_vis.shape = {keypoint_vis.shape}")
